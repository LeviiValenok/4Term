...
//            loadFromBinaryFile(*topOfTempStack);
            char nameOfTheFile[] = "Binary.bin";
            char letter, indicator;
            char* tempString;
            int i = 0;
            FILE *binaryFile;
            if (!(binaryFile = fopen(nameOfTheFile, "r + b")))
            {
...
//                fseek(binaryFile, 2, SEEK_CUR);
                ...
                tempString = "";
//                fread(&((*topOfStack)->name), 20, 1, binaryFile);
//                fread(&letter, sizeof(char), 1, binaryFile);
//                deleteSymbols(((*topOfStack))->name);
                ...
                {
//                    fgets(((*topOfStack))->information.color, 20, binaryFile);
//                    fread(&((*topOfStack)->information.color), 20, 1, binaryFile);
                    while (letter != ' ')
                    {
                        fread(&letter, sizeof(char), 1, binaryFile);
                        tempString[i] = letter;
