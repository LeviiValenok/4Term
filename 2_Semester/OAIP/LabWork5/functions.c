...
void loadFromFileToQueue(struct GeometricShapes **head, struct GeometricShapes** tail)
{
    switch(menuForLoadFromFile())
    {
        case 1:
        {
            ...
            {
                struct GeometricShapes* newItem;
                if (!(newItem = (struct GeometricShapes*)calloc(1, sizeof(struct GeometricShapes))))
                {
                    printf("Memory is not allocated\n");
                    return;
                }
                fscanf(textFile, "%d", &(newItem->square));
                if ((newItem)->square == 0)
                {
                    break;
                }
                fseek(textFile, 2, SEEK_CUR);
                fgets(newItem->name, 20, textFile);
                deleteSymbols(newItem->name);
                if ((newItem)->flag)
                {
                    fscanf(textFile, "%f", &(newItem)->information.perimeter);
                } else {
                    fgets((newItem)->information.color, 20, textFile);
                    deleteSymbols(newItem->information.color);
                }
                newItem->nextItem = NULL;
                if(*head == NULL)
